...
import { revokeTicket } from "@/lib/api";
import { useRouter } from "next/navigation";
import { SubmitButton } from "./Buttons";

export default function RevokeTicketForm({ bookedTicketId, ticketCode }: { bookedTicketId: number, ticketCode: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 mt-10 p-8 shadow-md border rounded-xl">
      <h2 className="text-2xl font-bold text-center text-gray-900">Revoke Ticket Quantity</h2>
      {error && (
      ...
      <div className="flex justify-center my-8">
        <SubmitButton label="Revoke" isPending={isPending} />
      </div>
    </form>
  );
};
